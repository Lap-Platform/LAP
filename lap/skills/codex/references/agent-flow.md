...
# Installs to ~/.codex/skills/stripe/
```
...
```bash
lapsh skill-install stripe --target codex --dir ./project-skills/stripe
```
...
# 3. The skill is now at ~/.codex/skills/stripe/
# Codex CLI auto-discovers it for Stripe API tasks
...
lapsh skill-install openai --target codex
```
