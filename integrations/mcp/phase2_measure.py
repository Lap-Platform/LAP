#!/usr/bin/env python3
"""Phase 2: Measure REAL token savings from ToolLean vs JSON Schema manifests."""
...
import tiktoken
from core.compilers.toollean import compile_mcp_manifest
...
for f in sorted(FIXTURES.glob("*.json")):
    ...
    toollean_text = bundle.to_toollean(lean=False)
    toollean_tokens = count_tokens(toollean_text)
    
    # Lean mode (no descriptions)
    toollean_lean = bundle.to_toollean(lean=True)
    toollean_lean_tokens = count_tokens(toollean_lean)
    ...
    results.append({
        ...
        "toollean": toollean_tokens,
        "toollean_lean": toollean_lean_tokens,
        "toollean_text": toollean_text,
        "json_text": json_text,
    })
    
    savings_vs_pretty = (1 - toollean_tokens / json_tokens) * 100
    savings_vs_compact = (1 - toollean_tokens / json_compact_tokens) * 100
    ...
    print(f"  ToolLean:       {toollean_tokens:>5} tokens ({savings_vs_pretty:.1f}% vs pretty, {savings_vs_compact:.1f}% vs compact)")
    print(f"  ToolLean lean:  {toollean_lean_tokens:>5} tokens")
...
total_toollean = sum(r["toollean"] for r in results)
total_lean = sum(r["toollean_lean"] for r in results)

print(f"Total JSON (pretty):   {total_pretty:>6} tokens")
print(f"Total JSON (compact):  {total_compact:>6} tokens")
print(f"Total ToolLean:        {total_toollean:>6} tokens  ({(1-total_toollean/total_pretty)*100:.1f}% savings vs pretty)")
print(f"Total ToolLean lean:   {total_lean:>6} tokens  ({(1-total_lean/total_pretty)*100:.1f}% savings vs pretty)")
print(f"\nSavings vs compact JSON: {(1-total_toollean/total_compact)*100:.1f}%")
...
print("\n--- ToolLean output ---")
print(gh["toollean_text"][:2000])

# Save data for results doc
with open(Path(__file__).parent / "phase2_data.json", "w") as f:
    json.dump([{k:v for k,v in r.items() if k not in ("toollean_text","json_text")} for r in results], f, indent=2)
